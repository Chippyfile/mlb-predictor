...
export default function MLBCalendarTab({ calibrationFactor, onGamesLoaded, onRefresh }) {
  ...
  const refreshGame = useCallback(async (game) => {
    ...
    try {
      ...
      // ATS sizing is written ONLY by the cron (main.py), which applies the rolling
      // CB+DM-boost screen (mlb_v12_paper_tiers.classify_paper_tier) — logic that can't
      // run client-side (no CSV, no classifier). Refresh no longer writes a second,
      // divergent ats_side/ats_units; it still refreshes ats_pick_status below.
      // [Option A follow-up] Have the backend predict endpoint return final
      // ats_side/ats_units/ats_v12_tier, then read mlResult.ats_* here to re-enable
      // refresh-time sizing from a single source of truth.
              ...
              {expanded === game.gamePk && (
                <div
                  ...
                  <div style={{
                    ...
                    {game.umpire?.name && <Kv k="Umpire" v={`${game.umpire.name}${game.umpire.home_win_pct ? ` (${(game.umpire.home_win_pct * 100).toFixed(0)}% home)` : game.umpire.size ? ` (${game.umpire.size})` : ''}`} />}
